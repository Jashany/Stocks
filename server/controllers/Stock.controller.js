...
const BuyStocksAtMarketPrice = async (req, res) => {
    try {
        ...
        const existingStock = user.stocks.find((s) => s.stockId.toString() === stockId);
...
const BuyStockFromAnotherUser = async (req, res) => {
    ...
    try {
       const { userId, TransactionId } = req.body;
        ...
        const existingStock = user.stocks.find((s) => s.stockId.toString() === transaction.stockId.toString());

        if (existingStock) {
            existingStock.quantity += transaction.quantity;
        } else {
            user.stocks.push({ stockId: transaction.stockId, quantity: transaction.quantity });
        }

        seller.money += totalPrice;

        const existingStockSeller = seller.stocks.find((s) => s.stockId.toString() === transaction.stockId.toString());
